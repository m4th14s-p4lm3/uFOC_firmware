...
The FOC module located in \texttt{uFOC\_lib/foc} directory implements functions 
for the Park and Clarke transformations as well as their inversions.
Furhter more a separate function for the \texttt{SVPWM} modulation is implemented.
To optimize trigonometric function execution speed, functions for sinus and cosinus calculations
ware implemented using lookup tables.

To encapsulate the FOC algorithm the \texttt{foc\_compute\_voltages} function was implemented,
to execute Park-Clarke transform PI current control regulation and then back the inverse Clarke-Park transformations.
