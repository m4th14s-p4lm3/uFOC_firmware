...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/curr_frame.png}
    \caption{Coordinate Transformations in FOC. \cite{simplefoc_diagram}}
    \label{fig:coordinate_frames}
\end{figure}

The diagram in Figure~\ref{fig:coordinate_frames} shows the principle of Clarke and Park transformations used in FOC.
Three phase stator currents are first projected from the $(a,b,c)$ coordinate system into the stationary $(\alpha,\beta)$ frame using Clarke transformation.
The stationary vectors are then rotated by the electrical rotor angle $\theta_e$ into the rotating $(d,q)$ reference frame using Park transformation.
This decomposition allows independent control of flux current $i_d$ and torque current $i_q$.

To control $i_d$ and $i_q$ currents, two independent proportionalintegral (PI) controllers are used to update applied voltage to the stator windings based on the error between the reference and the measured currents.
...
into two DC currents ($i_d$, $i_q$) in a rotating $dq$ reference frame from a rotor point of view, where $i_d$ is a flux current and $i_q$ is a torque current.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/svpwm.pdf}
    \caption{Modulation of a sinusoidal three-phase current using SVPWM.}
    \label{fig:svpwm}
\end{figure}
...
While encoder reading provides the mechanical angle of the motor shaft, the FOC algorithm operates with the electrical angle.
The number of electrical rotations per one mechanical rotation is directly proportional to the number of pole pairs in the rotor.
Therefore, the electrical angle $\theta_e$ can be expressed as

\begin{equation}
    \theta_e = P \cdot \theta_m
\end{equation}

where $P$ is the number of pole pairs and $\theta_m$ is the measured mechanical angle. \cite{electrical_offset}
