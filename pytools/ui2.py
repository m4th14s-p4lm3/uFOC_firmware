...
ENCODER_COUNTS_PER_MECH_REV = 1 << 21
...
class ElectricalOffsetDegreesRequest(BaseModel):
    degrees: float
    pole_pairs: int
...
@app.post("/encoder/electrical_offset_degrees")
def set_electrical_offset_degrees(body: ElectricalOffsetDegreesRequest):
    _check_rate()

    if body.pole_pairs <= 0:
        raise HTTPException(status_code=400, detail="Počet pólových párů musí být kladný.")
    if not math.isfinite(body.degrees):
        raise HTTPException(status_code=400, detail="Elektrický offset musí být konečné číslo.")
    if body.degrees < 0 or body.degrees > 360:
        raise HTTPException(status_code=400, detail="Elektrický offset musí být v rozsahu 0 až 360 stupňů.")

    counts_per_electrical_rev = ENCODER_COUNTS_PER_MECH_REV / body.pole_pairs
    raw_value = round((body.degrees / 360.0) * counts_per_electrical_rev)
    _get_client().set_electrical_offset(float(raw_value))
    return {
        "ok": True,
        "raw_value": raw_value,
        "degrees": body.degrees,
        "pole_pairs": body.pole_pairs,
    }
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .range-row { display: grid; gap: 6px; margin: 2px 0 10px; }
  .range-meta { display: flex; justify-content: space-between; color: var(--muted); font-size: 11px; }
...
<div class="grid">
  ...
  <div class="card">
    ...
    <h3>Elektrický offset <span class="unit">(° elektrické otáčky)</span></h3>
    <div class="read-row">
      <button class="neutral" onclick="getEncoderOffset()">Get</button>
      <div class="read-val" id="enc_off_r">—</div>
    </div>
    <div class="row">
      <div>
        <label>Pólové páry</label>
        <input id="enc_pole_pairs" type="number" min="1" value="7" step="1" oninput="updateElectricalOffsetPreview()">
      </div>
      <div>
        <label>Offset <span class="unit">°</span></label>
        <input id="enc_deg" type="number" min="0" max="360" value="0" step="0.1" oninput="syncElectricalOffset('number')">
      </div>
      <button onclick="setElectricalOffsetDegrees()">Set</button>
    </div>
    <div class="range-row">
      <input id="enc_deg_slider" type="range" min="0" max="360" value="0" step="0.1" oninput="syncElectricalOffset('slider')">
      <div class="range-meta"><span>0°</span><span>180°</span><span>360°</span></div>
    </div>
    <div class="read-row">
      <div class="read-val" id="enc_calc_r">raw: 0</div>
    </div>

    <hr class="divider">
    <h3>Raw offset <span class="unit">(21bit mechanická hodnota)</span></h3>
    <div class="row">
      <div><label>Offset</label><input id="enc_off" type="number" value="118107" step="1"></div>
      <button onclick="setRawEncoderOffset()">Set raw</button>
    </div>
...
/* ---- helpers ---- */
const ENCODER_COUNTS_PER_MECH_REV = 2 ** 21;

function numVal(id) { return parseFloat(document.getElementById(id).value); }

function finiteNumVal(id, label) {
  const value = numVal(id);
  if (!Number.isFinite(value)) throw new Error(label + ' musí být číslo.');
  return value;
}

function polePairsVal() {
  const value = parseInt(document.getElementById('enc_pole_pairs').value, 10);
  if (!Number.isInteger(value) || value < 1) {
    throw new Error('Počet pólových párů musí být kladné celé číslo.');
  }
  return value;
}

function electricalOffsetDegreesVal() {
  const value = finiteNumVal('enc_deg', 'Elektrický offset');
  if (value < 0 || value > 360) {
    throw new Error('Elektrický offset musí být v rozsahu 0 až 360°.');
  }
  return value;
}

function rawFromElectricalDegrees(degrees, polePairs) {
  return Math.round((degrees / 360) * (ENCODER_COUNTS_PER_MECH_REV / polePairs));
}

function electricalDegreesFromRaw(raw, polePairs) {
  const countsPerElectricalRev = ENCODER_COUNTS_PER_MECH_REV / polePairs;
  const wrapped = ((raw % countsPerElectricalRev) + countsPerElectricalRev) % countsPerElectricalRev;
  return (wrapped / countsPerElectricalRev) * 360;
}
...
/* ---- encoder ---- */
function updateElectricalOffsetPreview() {
  const el = document.getElementById('enc_calc_r');
  try {
    const polePairs = polePairsVal();
    const degrees = numVal('enc_deg');
    if (!Number.isFinite(degrees) || degrees < 0 || degrees > 360) {
      el.textContent = 'raw: —';
      return;
    }

    const raw = rawFromElectricalDegrees(degrees, polePairs);
    const electricalTurnCounts = Math.round(ENCODER_COUNTS_PER_MECH_REV / polePairs);
    el.textContent = 'raw: ' + raw + ' / 1 el. ot.: ' + electricalTurnCounts;
  } catch(e) {
    el.textContent = 'raw: —';
  }
}

function syncElectricalOffset(source) {
  const slider = document.getElementById('enc_deg_slider');
  const input = document.getElementById('enc_deg');
  const rawValue = source === 'slider' ? parseFloat(slider.value) : parseFloat(input.value);

  if (!Number.isFinite(rawValue)) {
    updateElectricalOffsetPreview();
    return;
  }

  const value = Math.min(360, Math.max(0, rawValue));
  if (source === 'slider') {
    input.value = formatDegrees(value);
  } else {
    slider.value = value;
  }
  updateElectricalOffsetPreview();
...
function setElectricalOffsetControls(degrees) {
  const value = Math.min(360, Math.max(0, degrees));
  document.getElementById('enc_deg').value = formatDegrees(value);
  document.getElementById('enc_deg_slider').value = value;
  updateElectricalOffsetPreview();
}

async function setElectricalOffsetDegrees() {
  if (!guardRate()) return;
  try {
    const degrees = electricalOffsetDegreesVal();
    const polePairs = polePairsVal();
    const d = await api('POST', '/encoder/electrical_offset_degrees', {
      degrees,
      pole_pairs: polePairs,
    });
    document.getElementById('enc_off').value = d.raw_value;
    document.getElementById('enc_off_r').textContent = Number(d.raw_value).toFixed(6);
    updateElectricalOffsetPreview();
    toast('Elektrický offset → ' + formatDegrees(degrees) + '° / raw ' + d.raw_value);
  } catch(e) { toast('✗ ' + e.message, 'err'); }
}

async function setRawEncoderOffset() {
  if (!guardRate()) return;
  try {
    const raw = finiteNumVal('enc_off', 'Raw offset');
    await api('POST', '/encoder/offset', { value: raw });
    document.getElementById('enc_off_r').textContent = raw.toFixed(6);
    try {
      setElectricalOffsetControls(electricalDegreesFromRaw(raw, polePairsVal()));
    } catch {}
    toast('Raw offset → ' + raw);
  } catch(e) { toast('✗ ' + e.message, 'err'); }
}

async function getEncoderOffset() {
  if (!guardRate()) return;
  try {
    const d = await api('GET', '/encoder/offset');
    const el = document.getElementById('enc_off_r');
    if (d.value === null || d.value === undefined) {
      el.textContent = 'null (timeout)';
      return;
    }

    const raw = Number(d.value);
    el.textContent = raw.toFixed(6);
    document.getElementById('enc_off').value = Math.round(raw);
    try {
      setElectricalOffsetControls(electricalDegreesFromRaw(raw, polePairsVal()));
    } catch {}
  } catch(e) { toast('✗ ' + e.message, 'err'); }
